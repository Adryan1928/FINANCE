...
def calcula_equilibrio_financeiro():
    saidas = Valores.objects.filter(data__month = datetime.now().month).filter(tipo = 'S')
    gastos_essenciais = saidas.filter(categoria__essencial = True)
    gastos_nao_essenciais = saidas.filter(categoria__essencial = False)
